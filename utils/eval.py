...
def validate(model: nn.Module, cfg: CONFIGCLASS):
    ...
    # print(y_true)
    # print(y_pred)
    # y_true_ = np.array(y_true).reshape(1, -1)  
    ...
    # # 注意：y_pred > 0.5 返回的是二维布尔数组，需要将其展平  
    # r_acc = accuracy_score(y_true[y_true == 0].flatten(), (y_pred[y_true == 0] > 0.5).flatten())  
    # f_acc = accuracy_score(y_true[y_true == 1].flatten(), (y_pred[y_true == 1] > 0.5).flatten())  
    # acc = accuracy_score(y_true.flatten(), (y_pred > 0.5).flatten())  
      
    # ap = average_precision_score(y_true.flatten(), y_pred.flatten())  
