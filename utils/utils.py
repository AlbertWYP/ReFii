...
from networks.unet_resnet import UNetResNet
...
def get_network(arch: str, args, isTrain=False, continue_train=False, init_gain=0.02, pretrained=True):
    if "resnet" in arch:
        from networks.resnet import ResNet
        model_unet, diffusion = create_model_and_diffusion(**args_to_dict(args,model_and_diffusion_defaults().keys()))
        resnet = getattr(import_module("networks.resnet"), arch)
        if isTrain:
            if continue_train:
                #print(111)
                ResNet = resnet(num_classes=1)# 为了测试做修改
                model = UNetResNet(model_unet,ResNet,diffusion)
            else:
                print(222)
                ResNet = resnet(pretrained=pretrained)
                model = UNetResNet(model_unet,ResNet,diffusion)
                # model.classifier[-1] = nn.Linear(in_features=model.classifier[-1].in_features, out_features=1, bias=True)
                # #model.fc = nn.Linear(model.fc.in_features, 1)
                # #model.fc = nn.Linear(1000, 1)
                # nn.init.normal_(model.fc.weight.data, 0.0, init_gain)
        else:
            ResNet = resnet(num_classes=1000)#为了测试做修改
            model = UNetResNet(model_unet,ResNet,diffusion)
        return model
    else:
