...
class BaseModel(nn.Module):
    ...
    def load_networks(self, epoch: int):
        ...
        state_dict = torch.load(load_path, map_location=self.device_ids[0])
            ...
            for state in self.optimizer.state.values():
                for k, v in state.items():
                    if torch.is_tensor(v):
                        state[k] = v.to(self.device_ids[0])
...
class Trainer(BaseModel):
    ...
    def __init__(self, cfg: CONFIGCLASS):
        ...
        self.arch = cfg.arch
        self.model = get_network(self.arch, cfg, cfg.isTrain, cfg.continue_train, cfg.init_gain, cfg.pretrained,)
        ...
        # self.model = self.model.to('cuda:' + str(self.device_ids[0]))    
        # # 使用DataParallel包装模型，并排除第0张卡  
        # self.model = nn.DataParallel(self.model, device_ids=self.device_ids)  
        # # 将模型移到第一张可用的GPU上（即cuda:1）
        
        # 检查可用的GPU数量    

        print(f"Using GPUs: {self.device_ids}")
        # 将模型移到第一个GPU上   
        self.model.to(self.device_ids[0])
        if len(self.device_ids) > 1:  
            self.model = nn.DataParallel(self.model, device_ids=self.device_ids)  # 使用多个GPU进行并行训练 
    ...
    def set_input(self, input):
        ...
        #img 大小 torch.Size([64, 3, 224, 224])
        #label 大小 torch.Size([64])
        # print("*****")
        # print(type(input))
        # print(type(img))
        # print(img.size())
        ...
        self.input = img.to(self.device_ids[0])# 将数据移到GPU上
        self.label = label.to(self.device_ids[0]).float()
        for k in meta.keys():
            if isinstance(meta[k], torch.Tensor):
                meta[k] = meta[k].to(self.device_ids[0])
        self.meta = meta

    def forward(self):
        self.output = self.model(self.input, self.meta)
        #print(self.model)
        # print(type(self.output)) 64 1000?
